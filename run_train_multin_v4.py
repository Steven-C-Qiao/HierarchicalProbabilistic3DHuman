...
#from data.flying_small_subset import OnTheFlySMPLTrainDataset
...
def run_train(device,
    ...
    blend_sigma = 1e-6
    blend_gamma = 1e-5
    smpl_faces = torch.from_numpy(smpl_model.faces[None].astype(np.int32)).to(device).expand(pose_shape_cfg.TRAIN.BATCH_SIZE, -1, -1)
    silh_renderer = SilhouetteRenderer(device=device,
                                        batch_size=pose_shape_cfg.TRAIN.BATCH_SIZE,
    ...
    criterion = MultinV4Loss(loss_config=pose_shape_cfg.LOSS.STAGE1,
                                        img_wh=pose_shape_cfg.DATA.PROXY_REP_SIZE)
