...
class MultinV4Loss(nn.Module):
    ...
    def __init__(self,
                 loss_config,
                 img_wh):
        super(MultinV4Loss, self).__init__()
        ...
        self.silh_loss = nn.MSELoss(reduction=loss_config.REDUCTION)
    ...
    def forward(self, target_dict, pred_dict):
        ...
        silh_loss = self.silh_loss(target_dict['target_silhs'], pred_dict['pred_silhs'])
        with open('./misc/losses.txt', 'a') as f:
            f.write(f'silh_loss: {silh_loss * self.loss_config.WEIGHTS.SILHOUETTE} ')
            f.write(f'verts_loss: {verts_loss * self.loss_config.WEIGHTS.VERTS3D} ')
            f.write(f'joint_2d_loss: {joints2D_loss * self.loss_config.WEIGHTS.JOINTS2D} \n')
        ...
        total_loss = pose_nll * self.loss_config.WEIGHTS.POSE \
                     ...
                     + silh_loss * self.loss_config.WEIGHTS.SILHOUETTE
