...
def predict_poseMF_shapeGaussian_net(pose_shape_model,
    ...
    i=0
    for image_fname in tqdm(['front_Apose.jpg', 'side_Apose.jpg', 'back_Apose.jpg']):#[f for f in os.listdir(image_dir) if f.endswith(('front_Apose.jpg', 'side_Apose.jpg', 'back_Apose.jpg'))]):
        with torch.no_grad():
    ...
    cropped_for_proxy_rgb = torch.nn.functional.interpolate(cropped_for_proxy['rgb'],
                                                            ...
                                                            mode='bilinear',
                                                            align_corners=False)
    body_vis_rgb = batch_add_rgb_background(backgrounds=cropped_for_proxy_rgb.repeat(3, 1, 1, 1),
    ...
    for i in range(batch_size):
        body_vis_output = body_vis_renderer(vertices=pred_vertices_mode[[i]],
                                        cam_t=cam_t[[i]],
                                        orthographic_scale=orthographic_scale[[i]],
                                        lights_rgb_settings=lights_rgb_settings,
                                        verts_features=vertex_var_colours[[i]])
        #body_vis_rgb = body_vis_rgb.detach().cpu().numpy()[i].transpose(1, 2, 0)
        #to_vis = body_vis_rgb[i]
        to_vis = body_vis_output['rgb_images'].detach().cpu().numpy()[i]
        print(to_vis.shape)
        ...
        # combined_vis_fig[visualise_wh:2*visualise_wh, i * visualise_wh: (i+1)*visualise_wh] = baseline_tovis
