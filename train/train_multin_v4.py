...
def gen_rotmat(rad, axis, aug=0.1):
    rad = torch.tensor(rad, dtype=float)
    rad += aug * torch.randn_like(rad)
    if axis=='x':
            ...
            for batch_num, samples_batch in enumerate(tqdm(dataloaders[split])):
                ...
                with torch.no_grad():
                    ...
                    for _ in range(int(pose_shape_cfg.TRAIN.BATCH_SIZE/3)):
                        front = gen_rotmat(0, 'x', 0.05) @ gen_rotmat(0, 'y') @ gen_rotmat(0, 'z', 0.05)
                        side =  gen_rotmat(0, 'x', 0.05) @ gen_rotmat(np.sign(np.random.randn(1)) * np.pi/2, 'y') @ gen_rotmat(0, 'z', 0.05)
                        back =  gen_rotmat(0, 'x', 0.05) @ gen_rotmat(np.pi, 'y') @ gen_rotmat(0, 'z', 0.05)
                    ...
                    target_silhs = iuv_in.sum(dim=1) # (B, 3, 256, 256)
                    target_silhs[target_silhs==-3] = 0 # due to the sum so -3
                    target_silhs[target_silhs!=0] = 1

                    if dev:
                        for iter in range(6):
                            silh_save = target_silhs[iter].cpu().detach().numpy()
                            silh_save = np.stack([silh_save]*3, axis=-1)  # single-channel to RGB
                            
                            vis_save_path = os.path.join('./misc/', 'silh' + str(iter) + '.png')
                            cv2.imwrite(vis_save_path, silh_save * 255)
                        # for iter in range(6):
                        #     iuv_save = iuv_in[iter].cpu().detach().numpy().transpose(1, 2, 0)
                        #     vis_save_path = os.path.join('./misc/', 'iuv' + str(iter) + '.png')
                        #     cv2.imwrite(vis_save_path, iuv_save * 255)
                    ...
                    # for iter in range(6):
                    #     proxy_save = proxy_rep_input[iter].sum(dim=0).cpu().detach().numpy()
                    #     proxy_save = np.stack([proxy_save]*3, axis=-1)  # single-channel to RGB
                        
                    #     vis_save_path = os.path.join('./misc/', 'proxy' + str(iter) + '.png')
                    #     cv2.imwrite(vis_save_path, proxy_save * 255)
                ...
                with torch.set_grad_enabled(split == 'train'):
                    ...
                    # ------------------------------ BUILD SILHOUETTE LOSS ------------------------------
                    orthographic_scale = pred_cam_wp[:, [0, 0]]
                    cam_t = torch.cat([pred_cam_wp[:, 1:],
                                        torch.ones(pred_cam_wp.shape[0], 1, device=device).float() * 2.5],
                                        dim=-1)
                    
                    pred_vertices_for_rend = aa_rotate_translate_points_pytorch3d(
                        points=pred_vertices_mode,
                        axes=x_axis,
                        angles=np.pi,
                        translations=torch.zeros(3, device=device).float())
                    pred_silhs = silh_renderer(vertices=pred_vertices_for_rend, 
                            cam_t = cam_t, 
                            orthographic_scale = orthographic_scale)['silhouettes']
                    
                    #pred_silhs = torch.flip(pred_silhs, (1,))

                    circles = pred_joints2d_coco_mode.cpu().detach().numpy()
                    
                    if dev:
                        for iter in range(6):
                            silh_save = pred_silhs[iter].cpu().detach().numpy()
                            ...
                            silh_save = np.stack([silh_save]*3, axis=-1)  # single-channel to RGB    
                            # for k in range(17):
                            #     silh_save = cv2.circle(silh_save, (circles[iter, k, 0], circles[iter, k, 1]), radius=0, color=(0, 0, 255), thickness=-1)
                            vis_save_path = os.path.join('./misc/', 'pred_silh' + str(iter) + '.png')
                            cv2.imwrite(vis_save_path, silh_save * 255)
                        dev = False
                    ...
                    target_dict_for_loss = {'pose_params_rotmats': target_pose_rotmats,
                                            ...
                                            'target_silhs': target_silhs}
                    # with open('./misc/joint.txt', 'a') as f:
                    #     f.write(f'pred_joints2D: {pred_joints2d_coco_samples[0, 0, 0, :]} ')
                    #     f.write(f'target_joints2D: {target_joints2d_coco[0, 0, :]} \n')
