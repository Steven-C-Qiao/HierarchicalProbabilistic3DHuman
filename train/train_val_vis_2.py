...
def train_val_vis(pose_shape_model,
    ...
    for batch_num, samples_batch in enumerate(tqdm(dataloaders['val'])):
        ...
        with torch.no_grad():
            ...
            target_reposed_vertices_flipped = aa_rotate_translate_points_pytorch3d(points=target_reposed_vertices,
            ...
            #pred_shape_dist.loc = pred_shape_dist.loc.mean(dim=0, keepdim=True).repeat(3, 1)

            print(pred_shape_dist.loc[0])

            pred_glob = pred_glob.repeat(2, 1, 1)
            pred_glob_rotmats = rot6d_to_rotmat(pred_glob)  # (bs, 3, 3)
            pred_glob_rotmats = pred_glob_rotmats[:3]
            ...
            print(f'pve: {np.sum(pve_batch)/(6890 * 3)}')  # scalar
            ...
            print(sum)
            print(f'pve-t-sc: {np.sum(pvet_sc_batch)/(6890*3)}')  # scalar
            ...
            print(np.sum(pve_sc_batch)/(6890*3))  # scalar
