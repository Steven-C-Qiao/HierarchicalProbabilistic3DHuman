...
def train_val_vis(pose_shape_model,
    ...
    for batch_num, samples_batch in enumerate(tqdm(dataloaders['val'])):
        ...
        with torch.no_grad():
            ...
            pred_reposed_vertices_flipped_mean = aa_rotate_translate_points_pytorch3d(points=pred_reposed_vertices,
                                                                                      axes=torch.tensor([1., 0., 0.], device=device),
                                                                                      angles=np.pi,
                                                                                      translations=torch.zeros(3, device=device))
            ...
            print(pred_reposed_vertices_sc.shape)
            pvet_sc_batch = np.linalg.norm((pred_reposed_vertices_sc) - (target_reposed_vertices).cpu().detach().numpy(), axis=-1)  # (bs, 6890)
            print(pvet_sc_batch.shape)
            sum = np.sum(pvet_sc_batch/(6890), axis=1)
            print(sum[:6])
            print(f'pve-t-sc: {np.sum(pvet_sc_batch)/(6890*pose_shape_cfg.TRAIN.BATCH_SIZE)}')  # scalar
            ...
            fixed_cam_t = torch.tensor([[0., -0.2, 2.5]], device=device).repeat(pose_shape_cfg.TRAIN.BATCH_SIZE, 1)
            ...
            target_reposed_vis_rgb = body_vis_renderer(vertices=target_reposed_vertices_flipped,
                                                     verts_features=vertex_var_colours,
                                                     cam_t=fixed_cam_t,
                                                     orthographic_scale=fixed_orthographic_scale,
                                                     lights_rgb_settings=lights_rgb_settings)['rgb_images'][0].cpu().detach().numpy()

            # per_vertex_3Dvar = np.linalg.norm((pred_reposed_vertices_sc) - (target_reposed_vertices).cpu().detach().numpy(), axis=-1)[0]  # (bs, 6890)
            # vertex_var_norm = plt.Normalize(vmin=0.0, vmax=0.2, clip=True)
            ...
            combined_vis_fig[:visualise_wh, 4*visualise_wh:5*visualise_wh] = target_reposed_vis_rgb
