...
class MultinV2(nn.Module):
    def __init__(self,
        ...
        theta1 = torch.tensor(0, dtype=float)
        theta2 = torch.tensor(np.pi/2, dtype=float)
        theta3 = torch.tensor(np.pi, dtype=float)
        R_y = torch.tensor([[[torch.cos(theta1), 0, torch.sin(theta1)],
                             [0, 1, 0],
                             [-torch.sin(theta1), 0, torch.cos(theta1)]],
                            [[torch.cos(theta2), 0, torch.sin(theta2)],
                             [0, 1, 0],
                             [-torch.sin(theta2), 0, torch.cos(theta2)]],
                            [[torch.cos(theta3), 0, torch.sin(theta3)],
                             [0, 1, 0],
                             [-torch.sin(theta3), 0, torch.cos(theta3)]],]).float()

        init_glob = rotmat_to_rot6d(torch.eye(3)[None, :].float())
        #init_glob = rotmat_to_rot6d(R_y) # (3, 6)
        ...
        assert self.config.MODEL.NUM_RESNET_LAYERS == 18
        self.image_encoder = resnet18(in_channels=self.config.MODEL.NUM_IN_CHANNELS,
                                        pretrained=False)
        self.encoder_1 = resnet18(in_channels=self.config.MODEL.NUM_IN_CHANNELS,
                                        pretrained=False)
        self.encoder_2 = resnet18(in_channels=self.config.MODEL.NUM_IN_CHANNELS,
                                        pretrained=False)
        self.encoder_3 = resnet18(in_channels=self.config.MODEL.NUM_IN_CHANNELS,
                                        pretrained=False)
        ...
        self.fc1_shape = nn.Linear(3 * num_image_features, fc1_dim)
    ...
    def forward(self, input, input_feats=None):
        ...
        # import os
        # for iter in range(6):
        #     proxy_save = input[3 * iter].sum(dim=0).cpu().detach().numpy()
        #     proxy_save = np.stack([proxy_save]*3, axis=-1)  # single-channel to RGB
            
        #     vis_save_path = os.path.join('./misc/', 'input' + str(iter) + '.png')
        ...
        if input_feats is None:
            ...
            feats_1 = self.encoder_1(input[ ::3, :, :, :])
            feats_2 = self.encoder_2(input[1::3, :, :, :])
            feats_3 = self.encoder_3(input[2::3, :, :, :])
        device = feats_1.device

        #input_feats = torch.cat([feats_1, feats_2, feats_3], dim=0).float().to(device)
        shape_feats = torch.cat([feats_1, feats_2, feats_3], dim=1).float().to(device)

        batch_size = input_feats.shape[0]
        
        x = self.activation(self.fc1(input_feats))
        x_shape = self.activation(self.fc1_shape(shape_feats))

        # Shape
        shape_params = self.fc_shape(x_shape)  # (bsize, num_smpl_betas * 2)
        ...
        # Glob rot and WP Cam
        # might be that nn.Linear cannot handle different sets of features, feat_1 ...?
        delta_cam = self.fc_cam(x)
        delta_glob = self.fc_glob(x)

        glob = delta_glob + self.init_glob #.repeat(int(batch_size/3), 1)  # (bsize, num glob)
