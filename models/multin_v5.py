...
# ONE ResEncoder, uniform shape from 3 views

class MultinV3(nn.Module):
    def __init__(self,
                 smpl_parents,
                 config):
        super(MultinV3, self).__init__()
        ...
        self.fc1_shape = nn.Linear(3 * num_image_features, fc1_shape_dim)

        self.fc_shape = nn.Linear(fc1_shape_dim, self.num_shape_params * 2)  # Means and variances for SMPL betas and/or measurements
    ...
    def forward(self, input, input_feats=None):
        ...
        shape_feats = input_feats.view(int(batch_size/3), 512 * 3)

        x = self.activation(self.fc1(input_feats)) # (bsize, 512)
