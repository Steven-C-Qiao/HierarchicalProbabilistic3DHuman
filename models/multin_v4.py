...
class MultinV2(nn.Module):
    def __init__(self,
                 ...
                 config):
        """
        Hypothesis: multin_v2 STACK feature vectors and feed to the same cam and 
        """
        ...
        self.num_cam_params = 3 * 3
        init_cam = torch.tensor([0.9, 0.0, 0.0]).float().repeat(3)  # Initialise weak-perspective camera scale at 0.9
    ...
    def forward(self, input, input_feats=None):
        ...
        input_feats = torch.cat([feats_1, feats_2, feats_3], dim=1).float().to(device)

        batch_size = input_feats.shape[0] * 3
